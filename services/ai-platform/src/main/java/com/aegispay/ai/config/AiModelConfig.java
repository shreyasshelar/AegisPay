 ...
 * <br><b>dev profile</b>: OpenRouter (via OpenAI-compatible adapter) is {@code @Primary}.
 *
...
public class AiModelConfig {
    ...
    /**
     * Anthropic Claude is the primary chat model on prod and any non-dev profile.
     * The {@code !dev} condition ensures this bean is NOT registered when
     * the dev profile is active, avoiding duplicate {@code @Primary} beans.
     */
    @Bean
    @Primary
    @Profile("!dev")
    ...
    // ── Dev — OpenRouter (OpenAI-compatible) ──────────────────────────────────

    /**
     * On dev profile, the OpenAI-adapter bean (pointing to OpenRouter) is primary.
     * {@code application-dev.yml} configures:
    ...
    @Primary
    @Profile("dev")
    public ChatModel primaryChatModelDev(OpenAiChatModel openAiChatModel) {
