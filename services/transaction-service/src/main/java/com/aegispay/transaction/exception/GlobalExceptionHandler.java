package com.aegispay.transaction.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleAuthentication(AuthenticationException ex) {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(ApiResponse.error(
                ErrorResponse.builder().errorCode("UNAUTHORIZED")
                        .message("Authentication required.").httpStatus(401).build()));
    }

    @ExceptionHandler(AccessDeniedException.class)
    public ResponseEntity<ApiResponse<Void>> handleAccessDenied(AccessDeniedException ex) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(ApiResponse.error(
                ErrorResponse.builder().errorCode("FORBIDDEN")
                        .message("Access denied.").httpStatus(403).build()));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleUnexpected(Exception ex) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ApiResponse.error(
                ErrorResponse.builder().errorCode("INTERNAL_SERVER_ERROR")
                        .message("An unexpected error occurred.").httpStatus(500).build()));
    }
}
