...
const KAFKA_FLOWS = [
  { from: "tx",      to: "kafka",   event: "transaction.initiated",      color: "#059669" },
  { from: "kafka",   to: "ledger",  event: "→ balance reservation",      color: "#059669" },
  { from: "kafka",   to: "risk",    event: "→ risk assessment",          color: "#dc2626" },
  { from: "kafka",   to: "orch",    event: "→ saga coordination",        color: "#8b5cf6" },
  { from: "kafka",   to: "notify",  event: "→ notifications",            color: "#0ea5e9" },
  { from: "kafka",   to: "pipeline",event: "→ analytics",                color: "#d97706" },
  { from: "risk",    to: "ai",      event: "RAG query",                  color: "#dc2626" },
  { from: "orch",    to: "notify",  event: "payment.completed",          color: "#059669" },
];

const TIER_LABELS = { client: "Client Tier", gateway: "Auth & Gateway", core: "Core Services", infra: "Event Bus", data: "Data Layer", analytics: "Analytics" };
const TIER_ORDER  = ["client", "gateway", "infra", "core", "data", "analytics"];
const TIER_COLORS = { client: "#0ea5e9", gateway: "#8b5cf6", core: "#059669", infra: "#dc2626", data: "#64748b", analytics: "#d97706" };

const TIER_GROUPS = TIER_ORDER.map(t => ({
  tier: t,
  services: Object.values(SERVICES).filter(s => s.tier === t),
}));
...
export default function AegisPaySystemMap() {
  ...
  const [activeTab, setActiveTab]   = useState("services"); // services | events | dataflow
  const [highlight, setHighlight]   = useState(null);

  const sel = selected ? SERVICES[selected] : null;

  const servicesByTier = (tier) =>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {svcs.map(svc => {
            ...
            const isHighlit  = highlight === svc.id;
            return (
              <button
                key={svc.id}
                onClick={() => setSelected(isSelected ? null : svc.id)}
                onMouseEnter={() => setHighlight(svc.id)}
                onMouseLeave={() => setHighlight(null)}
                style={{
                  background:    isSelected ? svc.color : (isHighlit ? svc.bg : "#fff"),
                  border:        `2px solid ${isSelected ? svc.color : (isHighlit ? svc.color : "#e2e8f0")}`,
                  borderRadius:  8,
                  padding:       "8px 14px",
                  cursor:        "pointer",
                  textAlign:     "left",
              ...
              >
                <div style={{ fontSize: 12, fontWeight: 700, color: isSelected ? "#fff" : "#0f172a", fontFamily: "'IBM Plex Sans', system-ui, sans-serif" }}>
                  {svc.label}
                </div>
                {svc.port && (
                  <div style={{ fontSize: 10, color: isSelected ? "rgba(255,255,255,0.8)" : "#94a3b8", fontFamily: "monospace", marginTop: 2 }}>
                    :{svc.port}
                  </div>
                )}
              </button>
      ...
      {[
        ...
        <div key={row.db} style={{ marginBottom: 12, padding: "10px 12px", background: "#f8fafc", borderRadius: 8, border: "1px solid #e2e8f0" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 6 }}>
            {row.dbs.map(d => (
              <span key={d} style={{ fontSize: 10, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 4, padding: "2px 6px", fontFamily: "monospace", color: "#475569" }}>{d}</span>
            ))}
          </div>
    ...
    <div style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif", maxWidth: 960, margin: "0 auto", padding: 24, background: "#fff" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 13, color: "#64748b", marginTop: 4 }}>
          10 microservices · Event-driven · SAGA orchestration · Double-entry ledger · AI-powered fraud detection
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 20 }}>
        {[
          ...
          { id: "events",    label: "⚡  Event Flows" },
          { id: "dataflow",  label: "🗄  Data Layer" },
        ].map(tab => (
          <button
            key={tab.id}
            ...
            style={{
              ...
              borderRadius: 8,
              border: "1.5px solid",
              borderColor: activeTab === tab.id ? "#6366f1" : "#e2e8f0",
      ...
      <div style={{ display: "grid", gridTemplateColumns: sel ? "1fr 340px" : "1fr", gap: 20 }}>
        {/* Left: service tiles or event/data panels */}
        <div>
          {activeTab === "services" && (
        ...
        {sel && (
          <div style={{ background: "#fff", borderRadius: 12, border: `2px solid ${sel.color}`, padding: 20, alignSelf: "start" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: sel.color, marginBottom: 4 }}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <div style={{ fontSize: 12, color: "#475569", marginTop: 4, lineHeight: 1.6 }}>
                {sel.tech.split(" / ").map(t => (
                  <span key={t} style={{ display: "inline-block", background: sel.bg, border: `1px solid ${sel.color}22`, borderRadius: 4, padding: "2px 7px", fontSize: 10, marginRight: 4, marginBottom: 4, color: sel.color, fontWeight: 600 }}>
                    {t}
                  </span>
                ))}
              </div>
      ...
      <div style={{ marginTop: 16, fontSize: 10, color: "#cbd5e1", textAlign: "center" }}>
        AegisPay v1.0 · 10 microservices · docs/interactive/aegispay_system_map.jsx
      </div>
    </div>
  );
}
