...
const STEPS = [
  {
    ...
    sublabel: "Input arrives from Risk Engine",
    color: "#6366f1",
    bg: "#eef2ff",
  },
  {
    id: 1,
    label: "Embed query",
    sublabel: "text-embedding-3-small → float[1536]",
    color: "#0891b2",
    bg: "#ecfeff",
  },
  {
    id: 2,
    label: "Vector search",
    sublabel: "pgvector HNSW cosine similarity",
    color: "#059669",
    bg: "#ecfdf5",
  },
  {
  ...
  },
  {
    ...
    sublabel: "claude-sonnet-4-6 → explanation",
    color: "#dc2626",
    bg: "#fef2f2",
  },
  {
...
const RETRIEVED_DOCS = [
  {
    ...
    content:
      "HIGH_VELOCITY + NEW_DEVICE: 5+ tx in 10 min from unknown device → 78% fraud rate in 23 confirmed cases. Risk contribution: +35 pts.",
  },
  {
    score: 0.87,
    category: "fraud_pattern",
    content:
      "NEW_PAYEE + HIGH_VALUE: First transaction to a new recipient over ₹10,000 carries elevated risk. Fraudsters exfiltrate via new payees.",
  },
  {
    score: 0.81,
    category: "fraud_pattern",
    content:
      "VELOCITY BURST pattern: >5 tx in any 10-min window is a hard trigger. Combined with device anomaly, escalate to REVIEW or BLOCK.",
  },
  {
    score: 0.72,
    category: "fraud_pattern",
    content:
      "DEVICE FINGERPRINT CHANGE mid-session: if user switches device mid-payment flow, treat as suspicious. Require step-up auth.",
  },
  {
    score: 0.68,
    category: "fraud_pattern",
    content:
      "ACCOUNT TAKEOVER signature: high velocity + new device + new payee is the classic ATO pattern. Block and notify via SMS immediately.",
  },
];
  ...
  return (
    <div style={{ fontFamily: "'JetBrains Mono', 'Fira Code', monospace", fontSize: 12, lineHeight: 1.7 }}>
      ...
      >
        ...
        {query.split("\n").map((line, i) => (
          <span key={i} style={{ display: "block", color: i === 0 ? "#f1f5f9" : "#94a3b8" }}>
            {line}
          </span>
        ))}
      </pre>
    ...
    <div>
      ...
      >
        ...
        <div style={{ fontSize: 11, color: "#64748b", marginTop: 4, width: "100%" }}>
          … 1536 dimensions total — showing first 64 · hover for value · red = positive, blue = negative
        </div>
      </div>
      <div
        style={{
  ...
  return (
    <div>
      ...
      >
        ...
        <span style={{ color: "#f1f5f9" }}>content, metadata, </span>
        <span style={{ color: "#fbbf24" }}>embedding {"<=>"} $queryVec</span>
        <span style={{ color: "#f1f5f9" }}> AS distance{"\n"}</span>
        <span style={{ color: "#7dd3fc" }}>FROM </span>
        <span style={{ color: "#f1f5f9" }}>knowledge_base{"\n"}</span>
        <span style={{ color: "#7dd3fc" }}>WHERE </span>
        <span style={{ color: "#f1f5f9" }}>category = </span>
        <span style={{ color: "#86efac" }}>'fraud_pattern'</span>
        <span style={{ color: "#94a3b8" }}>{"\n"}</span>
        <span style={{ color: "#7dd3fc" }}>ORDER BY </span>
        <span style={{ color: "#fbbf24" }}>distance ASC{"\n"}</span>
        <span style={{ color: "#7dd3fc" }}>LIMIT </span>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {RETRIEVED_DOCS.map((doc, i) => (
          <div
            ...
            style={{
              border: "1px solid",
              borderColor: doc.score > 0.85 ? "#6ee7b7" : doc.score > 0.75 ? "#fcd34d" : "#e2e8f0",
              borderRadius: 8,
              padding: "10px 14px",
              background: doc.score > 0.85 ? "#f0fdf4" : doc.score > 0.75 ? "#fffbeb" : "#f8fafc",
            }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
              ...
              <span
                style={{
                  fontSize: 11,
                  fontWeight: 700,
                  color: doc.score > 0.85 ? "#059669" : doc.score > 0.75 ? "#d97706" : "#64748b",
                  fontFamily: "monospace",
                }}
              >
                similarity: {doc.score.toFixed(2)}
              </span>
            </div>
            ...
            <div style={{ marginTop: 6, height: 3, borderRadius: 2, background: "#e2e8f0" }}>
              <div
                style={{
                  ...
                  height: "100%",
                  borderRadius: 2,
                  background: doc.score > 0.85 ? "#10b981" : doc.score > 0.75 ? "#f59e0b" : "#94a3b8",
                }}
              />
            </div>
          </div>
  ...
  return (
    ...
    >
      <div style={{ marginBottom: 10 }}>
        <span style={{ background: "#1e293b", color: "#7dd3fc", padding: "2px 8px", borderRadius: 4, fontSize: 10, fontWeight: 600 }}>
          SYSTEM
        </span>
      </div>
      <div style={{ color: "#94a3b8", marginBottom: 14 }}>
        You are AegisPay's financial assistant. Use ONLY the provided context. Do not hallucinate.
        Cite specific patterns. Always give a recommended action.
      </div>
      <div style={{ marginBottom: 10 }}>
        <span style={{ background: "#1e293b", color: "#86efac", padding: "2px 8px", borderRadius: 4, fontSize: 10, fontWeight: 600 }}>
      ...
      {RETRIEVED_DOCS.slice(0, 3).map((doc, i) => (
        <div
          key={i}
          style={{ color: "#64748b", marginBottom: 8, paddingLeft: 12, borderLeft: "2px solid #1e293b" }}
        >
          [{i}] {doc.content}
        </div>
      ))}
      <div style={{ color: "#94a3b8", marginTop: 8 }}>
        ...
        Transaction flagged riskScore=78, flags=HIGH_VELOCITY,NEW_DEVICE, amount=15000 INR...
    ...
    if (isBold) {
      ...
      return (
        <span key={idx}>
          {parts.map((p, j) =>
            j % 2 === 1 ? (
              <strong key={j} style={{ color: "#1e293b" }}>
                {p}
              </strong>
            ) : (
              p
    ...
    <div style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif", padding: "24px 20px", maxWidth: 780, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 13, color: "#64748b", marginTop: 2 }}>
          Fraud Copilot · follows this exact path for every flagged transaction
        </div>
      </div>
        ...
        {STEPS.map((s, i) => (
          <button
            ...
            style={{
              ...
              minWidth: 100,
              border: "none",
              borderBottom: `3px solid ${active === i ? s.color : visited.has(i) ? "#e2e8f0" : "#f1f5f9"}`,
          ...
          >
            <div style={{ fontSize: 10, fontWeight: 700, color: active === i ? s.color : "#94a3b8", marginBottom: 2 }}>
              {String(i + 1).padStart(2, "0")}
            </div>
            <div style={{ fontSize: 11, fontWeight: 600, color: active === i ? "#0f172a" : "#475569" }}>
              {s.label}
            </div>
          </button>
        ))}
          ...
          <span key={i} style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <span
              style={{
                ...
                height: 20,
                borderRadius: "50%",
                background: visited.has(i) ? s.color : "#e2e8f0",
                color: visited.has(i) ? "#fff" : "#94a3b8",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                fontSize: 10,
