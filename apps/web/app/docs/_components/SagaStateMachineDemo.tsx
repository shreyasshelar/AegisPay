...
const STATES: Record<string, { label: string; color: string; desc: string; x: number; y: number }> = {
...
const SCENARIOS = [
  { id: "happy",   label: "✅ Happy Path",           color: "#059669", path: ["STARTED","FUNDS_RESERVED","RISK_ASSESSED","PAYMENT_PROCESSING","PAYMENT_COMPLETED","LEDGER_COMMITTED","COMPLETED"], desc: "Payment flows through all steps without errors." },
  { id: "insuf",   label: "💸 Insufficient Funds",   color: "#dc2626", path: ["STARTED","FAILED"],                                                                                                   desc: "Ledger rejects reservation immediately. Saga fails at step 1 — no compensation needed." },
  { id: "risk",    label: "🚨 Risk Blocked",          color: "#dc2626", path: ["STARTED","FUNDS_RESERVED","COMPENSATING","FUNDS_RELEASED","FAILED"],                                                  desc: "Risk engine issues BLOCK (score ≥ 80). Orchestrator releases reservation as compensation." },
  { id: "stripe",  label: "💳 Stripe Error",          color: "#d97706", path: ["STARTED","FUNDS_RESERVED","RISK_ASSESSED","PAYMENT_PROCESSING","COMPENSATING","FUNDS_RELEASED","FAILED"],             desc: "Stripe returns error (card_declined, amount_too_small). Reservation released. No ledger entries written." },
  { id: "timeout", label: "⏱ Saga Timeout",          color: "#d97706", path: ["STARTED","FUNDS_RESERVED","RISK_ASSESSED","PAYMENT_PROCESSING","COMPENSATING","FUNDS_RELEASED","FAILED"],             desc: "Saga exceeds 5-min timeout. SagaTimeoutAlert fires. Compensation chain triggered automatically." },
];
        ...
        return (
          <div key={sid} style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            <div
              ...
              style={{ padding: "8px 20px", borderRadius: 10, border: `2px solid ${isActive ? s.color : (isDone ? s.color + "80" : "#e2e8f0")}`, background: isActive ? s.color : (isDone ? s.color + "15" : "#fff"), color: isActive ? "#fff" : (isDone ? s.color : "#94a3b8"), fontSize: 12, fontWeight: isActive ? 700 : 500, fontFamily: "monospace", cursor: "pointer", transition: "all 0.15s", minWidth: 180, textAlign: "center", boxShadow: isActive ? `0 2px 12px ${s.color}40` : "none" }}
            >
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif", maxWidth: 860, margin: "0 auto", padding: 24 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
        {SCENARIOS.map(s => (
          <button key={s.id} onClick={() => { setScenario(s.id); setStepIdx(0); }} style={{ padding: "6px 14px", borderRadius: 8, border: `2px solid ${scenario === s.id ? s.color : "#e2e8f0"}`, background: scenario === s.id ? s.color : "#fff", color: scenario === s.id ? "#fff" : "#475569", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
            {s.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 20 }}>
        ...
        <div>
          <div style={{ background: "#fff", borderRadius: 12, border: `2px solid ${curState.color}`, padding: 20, marginBottom: 14 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: "#0f172a", fontFamily: "monospace", marginBottom: 12 }}>{current}</div>
