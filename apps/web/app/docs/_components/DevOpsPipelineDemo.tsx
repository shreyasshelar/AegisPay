...
const CI_STEPS = [
  { id: 1, phase: "Trigger",  actor: "GitHub Actions", color: "#6366f1", title: "Push to main / dev", detail: "Push triggers ci-java.yml. dorny/paths-filter detects which services changed. Only changed services (+ anything that imports a changed lib) run their build job — saves 60-80% CI minutes.", code: `on:\n  push:\n    branches: [main, dev]\n    paths: ['libs/**', 'services/**']\n\n# detect-changes job outputs:\n#   transaction-service: true\n#   ledger-service: false  ← not rebuilt` },
  { id: 2, phase: "CI",       actor: "Maven + JUnit 5", color: "#059669", title: "Build, test, verify", detail: "Each changed service: mvn verify (compile + unit tests + Checkstyle). Skips Docker build on dev branch — only main gets pushed to GHCR to save registry costs.", code: `mvn --batch-mode -pl services/transaction-service\n  -am verify\n  -Dcheckstyle.failOnViolation=true\n  -Dsurefire.failIfNoSpecifiedTests=false\n\n# Layer-cached Docker build (only on main):\ndocker buildx build --cache-from type=gha\n  --tag ghcr.io/.../transaction-service:$SHA` },
  { id: 3, phase: "CI",       actor: "GHCR", color: "#0891b2", title: "Push layered image to GHCR", detail: "Spring Boot layered JAR: dependencies / spring-boot-loader / snapshot-dependencies / application. Only the 'application' layer changes per build — pulls from cache. Non-root user 1001 (aegispay) baked in.", code: `# Dockerfile: layered extraction\nFROM eclipse-temurin:21-jre-jammy AS builder\nRUN java -Djarmode=layertools -jar app.jar extract\n\nFROM eclipse-temurin:21-jre-jammy\nUSER aegispay  # non-root UID 1001\nCOPY --from=builder /app/dependencies/ ./\nCOPY --from=builder /app/application/ ./` },
  { id: 4, phase: "Security", actor: "Trivy + CodeQL", color: "#dc2626", title: "Security scans (parallel)", detail: "Trivy: scans all 10 service images for CRITICAL+HIGH CVEs, uploads SARIF to GitHub Security tab. CodeQL: Java static analysis. OWASP dep-check: fails on CVSS ≥ 7. Runs weekly + on every main push.", code: `trivy-action:\n  image-ref: ghcr.io/.../transaction-service:latest\n  severity: CRITICAL,HIGH\n  format: sarif\n  exit-code: "0"  # advisory, not blocking\n\ncodeql:\n  languages: java\n\nowasp: -DfailBuildOnCVSS=7` },
  { id: 5, phase: "CD",       actor: "cd-prod.yml", color: "#8b5cf6", title: "Update values-prod.yaml", detail: "cd-prod.yml triggers after Java CI succeeds on main. Validates all 10 images exist in GHCR (docker manifest inspect). Updates values-prod.yaml image tags using yq. Commits [skip ci] back to main.", code: `for svc in $JAVA_SERVICES; do\n  yq e ".services.${'{svc}'}.image.tag = \\"${'{SHA}'}\\""  \\\n    -i values-prod.yaml\ndone\n\ngit commit -m "chore(cd/prod): deploy ${'{SHA}'.slice(0, 12)} [skip ci]"\ngit push origin main` },
  { id: 6, phase: "CD",       actor: "ArgoCD", color: "#d97706", title: "ArgoCD detects OutOfSync → syncs", detail: "ArgoCD watches the git repo. When values-prod.yaml changes, app becomes OutOfSync. ArgoCD runs 'helm template' against the new values and applies the diff to k8s. Waits for health=Healthy.", code: `argocd app sync aegispay-prod \\\n  --auth-token $ARGOCD_TOKEN \\\n  --server $ARGOCD_SERVER \\\n  --grpc-web --timeout 300\n\nargocd app wait aegispay-prod \\\n  --health --timeout 300\n\n# Post-deploy smoke:\ncurl --fail ${'{GATEWAY_URL}'}/actuator/health` },
  { id: 7, phase: "Deploy",   actor: "Kubernetes", color: "#059669", title: "Rolling update — zero downtime", detail: "k8s rolling update replaces pods one at a time. topologySpreadConstraints ensure pods spread across nodes. terminationGracePeriodSeconds=30 lets Spring drain in-flight requests gracefully before kill.", code: `# All deployments: runAsNonRoot, readOnlyRootFilesystem\n# seccompProfile: RuntimeDefault\n# capabilities: drop: [ALL]\n\n# Liveness: /actuator/health/liveness\n# Readiness: /actuator/health/readiness\n# initialDelaySeconds: 20-30 (JVM warmup)` },
];
...
const ENVS = [
  { id: "local", name: "Local", subtitle: "Full stack, zero cost", icon: "💻", color: "#64748b", trigger: "Manual — docker compose up", branch: null, argocd: false, hpa: false, tls: false, pdb: false, replicas: "1 each", infra: "Docker Compose (no k8s)", secrets: ".env.local — plaintext, never committed", cost: "~₹0 / month", costNote: "Everything runs on your laptop. No cloud spend.", gates: [], desc: "All 10 services + all infra via Docker Compose. Seed scripts inject test users and transactions. Port names match k8s service names so config is shared.", values: "docker-compose.yml + .env.local", integrations: [{ name: "AI / LLM", value: "Anthropic direct (dev key, low RPM limit)", cost: "free tier" }, { name: "Payments", value: "Stripe test mode (pk_test_...)", cost: "free" }, { name: "Auth", value: "Keycloak local container :8180", cost: "free" }, { name: "SMS", value: "Fast2SMS sandbox — no real SMS sent", cost: "free" }, { name: "Email", value: "Gmail SMTP dev account", cost: "free" }] },
  { id: "dev",   name: "Dev",   subtitle: "Cost-optimised cloud — real infra, sandbox integrations", icon: "🔧", color: "#0891b2", trigger: "Auto — push to dev branch → CI green", branch: "dev", argocd: true, hpa: false, tls: true, pdb: false, replicas: "1 each (no HA)", infra: "EKS t3.medium × 2 nodes  (or k3s single-node)", secrets: "AWS Secrets Manager — free tier (< 100 secrets, 10k API calls/mo)", cost: "~₹2,500–3,500 / month", costNote: "2 × t3.medium + 1 × RDS t3.micro + ElastiCache. OpenRouter keeps AI cost near zero during dev iteration.", gates: ["CI green (ci-java.yml + ci-web.yml)", "Auto-sync ArgoCD (no approval)"], desc: "Mirrors prod topology exactly — same Helm chart, same ArgoCD ApplicationSet, same NetworkPolicies. Only the integration endpoints and replica counts differ. Auto-deploys on every green push to dev. No PDB since single replica.", values: "values-dev.yaml (committed)", integrations: [{ name: "AI / LLM", value: "OpenRouter (claude-haiku-3 + fallbacks)", cost: "~$0.25/1M tokens  ≈ 10× cheaper than direct Anthropic" }, { name: "Payments", value: "Stripe test mode — no real money moves", cost: "free" }, { name: "Auth", value: "Keycloak pod in-cluster", cost: "included in node cost" }, { name: "Kafka", value: "Single-broker in-cluster (1 partition)", cost: "included" }, { name: "Postgres", value: "Single RDS t3.micro (Multi-AZ off)", cost: "~$15/mo" }] },
  { id: "prod",  name: "Prod",  subtitle: "Full HA — real integrations, approval gate", icon: "🚀", color: "#059669", trigger: "Manual gate — PR to main → CI green → GitHub Env approval → deploy", branch: "main", argocd: true, hpa: true, tls: true, pdb: true, replicas: "2–10 (HPA)", infra: "EKS m5.large × 3+ nodes (Multi-AZ)", secrets: "HashiCorp Vault — k8s service account auth (no static tokens)", cost: "~₹35,000–50,000 / month", costNote: "EKS node group + RDS Multi-AZ + ElastiCache + ClickHouse. HPA keeps costs proportional to load.", gates: ["CI green (all workflows)", "GitHub Environment approval (manual)", "CD smoke test /actuator/health"], desc: "PDB ensures min 1 pod always available during rolling deploys. topologySpreadConstraints spread pods across AZs. CD smoke test fails fast if the deploy is broken. ArgoCD manual sync (not auto) for safety.", values: "values-prod.yaml (image tags updated by ci-cd bot, committed with [skip ci])", integrations: [{ name: "AI / LLM", value: "Anthropic direct (claude-haiku + sonnet)", cost: "pay-per-token, full rate limits" }, { name: "Payments", value: "Stripe live mode (pk_live_...)", cost: "Stripe fees apply" }, { name: "Kafka", value: "3-broker cluster, RF=3, 3 partitions each", cost: "included" }, { name: "Postgres", value: "RDS r6g.large Multi-AZ per service DB", cost: "~$200/mo (8 DBs)" }, { name: "Redis", value: "ElastiCache r6g.large cluster mode", cost: "~$80/mo" }] },
];

type TabId = "cicd" | "k8s" | "security" | "observ" | "secrets" | "envs";

export default function DevOpsPipelineDemo() {
  const [tab,         setTab]         = useState<TabId>("cicd");
  ...
  const [activeEnvTab, setActiveEnvTab] = useState<"integrations" | "infra" | "gates">("integrations");
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif", maxWidth: 960, margin: "0 auto", padding: 24 }}>
      ...
      <div style={{ marginBottom: 22 }}>
        ...
        <div style={{ fontSize: 13, color: "#64748b", marginTop: 3 }}>GitHub Actions → GHCR → ArgoCD → EKS/k3s · 3 environments · 10 services · Security-hardened</div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10, marginBottom: 22 }}>
        {[
          ...
          { label: "Prometheus alerts", value: "10", color: "#d97706" },
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 20, flexWrap: "wrap" }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id as TabId)} style={{ padding: "7px 14px", borderRadius: 8, border: `1.5px solid ${tab === t.id ? "#6366f1" : "#e2e8f0"}`, background: tab === t.id ? "#6366f1" : "#fff", color: tab === t.id ? "#fff" : "#475569", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 16 }}>
          <div style={{ background: "#f8fafc", borderRadius: 12, padding: 12, border: "1px solid #e2e8f0" }}>
            ...
            {CI_STEPS.map((s, i) => {
              ...
              return (
                <div key={s.id} onClick={() => setCiStep(i)} style={{ display: "flex", gap: 8, padding: "8px", borderRadius: 7, marginBottom: 4, cursor: "pointer", background: isActive ? s.color + "15" : "transparent", border: `1.5px solid ${isActive ? s.color : "transparent"}`, transition: "all 0.12s" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
              ...
              <tbody>
                {K8S_RESOURCES.map((svc, i) => (
                  <tr key={svc.name} onClick={() => setSelectedSvc(selectedSvc === i ? null : i)} style={{ borderBottom: "1px solid #f1f5f9", cursor: "pointer", background: selectedSvc === i ? "#f0f9ff" : (i % 2 === 0 ? "#fff" : "#fafafa") }}>
                    ...
                    {[svc.hpa, svc.pdb, svc.np, svc.ingress].map((v, j) => (
                      <td key={j} style={{ padding: "9px 10px", textAlign: "center", fontSize: 14 }}>{v ? "✅" : "—"}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
      ...
      {tab === "security" && (
        <div>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
            {secCats.map(c => (
              <button key={c} onClick={() => setSecCat(c)} style={{ padding: "5px 12px", borderRadius: 20, border: `1.5px solid ${secCat === c ? "#059669" : "#e2e8f0"}`, background: secCat === c ? "#059669" : "#fff", color: secCat === c ? "#fff" : "#475569", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>{c}</button>
            ))}
        ...
        return (
          <div>
            <div style={{ background: "#f8fafc", borderRadius: 14, padding: "20px 20px 16px", border: "1px solid #e2e8f0", marginBottom: 20 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 60px 1fr 60px 1fr", alignItems: "start", gap: 0 }}>
                {ENVS.map((e, idx) => {
                  const isSelected = selectedEnv === e.id;
                  const isLast = idx === ENVS.length - 1;
                  return [
                    <div key={e.id} onClick={() => { setSelectedEnv(e.id); setActiveEnvTab("integrations"); }} style={{ background: "#fff", borderRadius: 12, border: `2px solid ${isSelected ? e.color : "#e2e8f0"}`, padding: "14px 16px", cursor: "pointer", transition: "all 0.18s", boxShadow: isSelected ? `0 2px 16px ${e.color}25` : "none" }}>
                      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 10 }}>
                        <div>
                      ...
                      <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 10 }}>
                        {[
                          ...
                          <span key={b.label} style={{ fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 10, background: b.on ? b.bg : "#f8fafc", color: b.on ? b.color : "#cbd5e1", border: `1px solid ${b.on ? b.border : "#e2e8f0"}` }}>{b.label}</span>
                        ))}
                      </div>
                    ...
                    !isLast && (
                      <div key={`arrow-${idx}`} style={{ display: "flex", alignItems: "center", justifyContent: "center", paddingTop: 50 }}>
            ...
            {env && (
              <div style={{ background: "#fff", borderRadius: 14, border: `2px solid ${env.color}`, padding: 20, marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
                  {([{ id: "integrations", label: "🔌 Integrations" }, { id: "infra", label: "☸️ Infra" }, { id: "gates", label: "⚑ Promotion gates" }] as const).map(t => (
