...
type Account  = typeof INITIAL.payer;
type Entry    = { id: number; txId: number; account: string; type: "DEBIT" | "CREDIT"; amount: number; currency: string };
type HistItem = { txId: number; amount: number; type: string; ts: string };
type LogItem  = { msg: string; color: string; ts: string };

function BalanceBar({ account, name }: { account: Account; name: string }) {
...
export default function LedgerExplorerDemo() {
  ...
  const addLog = (msg: string, color = "#374151") =>
    setLog(l => [{ msg, color, ts: new Date().toLocaleTimeString() }, ...l].slice(0, 12));

  const reserve = () => {
    if (accounts.payer.available < amount) {
      addLog(`❌ INSUFFICIENT_FUNDS — available ₹${accounts.payer.available} < ₹${amount}`, "#dc2626");
      setPhase("failed");
      return;
    }
    setAccounts(a => ({ ...a, payer: { ...a.payer, available: a.payer.available - amount, reserved: a.payer.reserved + amount, version: a.payer.version + 1 } }));
    addLog(`✅ Phase 1 RESERVE — ₹${amount} moved from available → reserved (tx-${txId})`, "#0891b2");
    setPhase("reserved");
  };

  const commit = () => {
    const debit:  Entry = { id: entries.length + 1, txId, account: "payer", type: "DEBIT",  amount, currency: "INR" };
    const credit: Entry = { id: entries.length + 2, txId, account: "payee", type: "CREDIT", amount, currency: "INR" };
    setEntries(e => [...e, debit, credit]);
    setAccounts(a => ({
      payer: { ...a.payer, reserved: a.payer.reserved - amount, version: a.payer.version + 1 },
      payee: { ...a.payee, available: a.payee.available + amount, version: a.payee.version + 1 },
    }));
  ...
  const fullReset = () => { setAccounts({ ...INITIAL }); setEntries([]); setPhase("idle"); setTxId(1); setLog([]); setHistory([]); };
    ...
    <div style={{ fontFamily: "'IBM Plex Sans', system-ui, sans-serif", maxWidth: 900, margin: "0 auto", padding: 24 }}>
      ...
      <div style={{ background: "#f8fafc", borderRadius: 12, padding: 16, border: "1px solid #e2e8f0", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 200, background: "#fff", borderRadius: 10, padding: "12px 14px", border: phase === "idle" ? "2px solid #0891b2" : "2px solid #e2e8f0" }}>
            ...
            <button onClick={reserve} disabled={phase !== "idle"} style={{ width: "100%", padding: "8px", borderRadius: 8, border: "none", background: phase === "idle" ? "#0891b2" : "#e2e8f0", color: phase === "idle" ? "#fff" : "#94a3b8", fontSize: 12, fontWeight: 700, cursor: phase === "idle" ? "pointer" : "not-allowed" }}>
          ...
          <div style={{ flex: 1, minWidth: 200, background: "#fff", borderRadius: 10, padding: "12px 14px", border: phase === "reserved" ? "2px solid #059669" : "2px solid #e2e8f0" }}>
            ...
            <button onClick={commit} disabled={phase !== "reserved"} style={{ width: "100%", padding: "8px", borderRadius: 8, border: "none", background: phase === "reserved" ? "#059669" : "#e2e8f0", color: phase === "reserved" ? "#fff" : "#94a3b8", fontSize: 12, fontWeight: 700, cursor: phase === "reserved" ? "pointer" : "not-allowed" }}>
              Commit (Stripe ✓)
            </button>
          </div>
          <div style={{ flex: 1, minWidth: 200, background: "#fff", borderRadius: 10, padding: "12px 14px", border: phase === "reserved" ? "2px solid #d97706" : "2px solid #e2e8f0" }}>
            ...
            <button onClick={release} disabled={phase !== "reserved"} style={{ width: "100%", padding: "8px", borderRadius: 8, border: "none", background: phase === "reserved" ? "#d97706" : "#e2e8f0", color: phase === "reserved" ? "#fff" : "#94a3b8", fontSize: 12, fontWeight: 700, cursor: phase === "reserved" ? "pointer" : "not-allowed" }}>
              Release (Stripe ✗)
            </button>
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={{ background: "#f8fafc", borderRadius: 12, padding: 14, border: "1px solid #e2e8f0" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.06em" }}>Ledger Entries (append-only)</div>
          ...
          {entries.length === 0 ? (
            ...
            <div style={{ maxHeight: 240, overflow: "auto" }}>
              ...
              <div style={{ marginTop: 8, padding: "6px", display: "flex", gap: 16 }}>
